...
**ADOSEDUR** 
is used to determine the appropriate start imputation (time?) for
missing C0 values in intravascular doses (IV). The automatic half-life
...
The automatic HL calculation will never include points that are still
hapenning during an infussion administration, this is what is meant
here. On top, if an administration is bolus the C0 is extrapolated
with a log-lin back regression. And yes, the variable ADOSEDUR will be
the administration duration (so only infusions will be ADOSEDUR>0)
