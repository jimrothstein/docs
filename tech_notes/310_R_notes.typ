...
Ch 6 in Advanced R; possible Ch 18 (AST)
SEE:  https://colinfay.me/r-assignment/

compare:   f(x=4) and f(x <-4);  not the same!
f(x <-4) creates variable x in calling env. evaluates and sets its value to 4.
f(x = 4) does not touch global env
...
(Rmk:   .Call() is unrelated, calls C function).  The form `do.call(f, args) is useful when programming. `args` is list of arguments, plus things like rm.na

### create a call

**create a call**
-   See:  R-lang 6.5, call, sys.call vs match.call
-   See:  Ch 18.3 `call objects`
...
Like symbol, expression, call is typeof 'language'-- avoid!  SEE Adv-R Ch 18
```
...
**byte code** (TODO) "Readable" concise instructions; not machine code; no user
access; use JIT compiler or interpeter.<bytecode: 0x1327...>   
(TODO) Compare this to r-wasm.  Both start with AST, but one builds, compiles into `assembler` code.
...
**expression**
Unevaluated code.
The correct interpretation of expression, also Hadley's, is
any set of base objects (scalar, constant, symbol, call objects, pairlist and ....)
...
**expression vector**
R is sloppy about expression.  

(1) Sometimes it agrees with Hadley's definition.
(2) But it is also one of the typeof() called `expression`, Hadley calls this `expression vectors` and is not what Hadley means by `expression`

is.expression() T means it is `expression vector`, list-like
parse() and expression() return object of type `expression` (ie is.expression() = T)
...
```
**eval()**
Note:  rlang::expr(x) quotes (or converts ) arg to expression
```{r eval}
x = 2
y = 10
try ( 
...
**Evaluation**  (parse, deparse), evaluator SEE R-lang definitions 2.1.3 
```
...
**DO NOT CONFUSE call and expression**
An expression also a formal language construction with list structure.  The function expression() takes an R
object and returns expression object.  

**STUDY** Compare q1 (a call) , q2 (expression object)
```r
# call
q1 = quote(a+b)
...
Note:  a `statement` is code that may do things
...
**Function**
- formals(f) returns formal arguments of function, with default values, but NO evaluation, NO look in environment.  (SEE pairlist)
...
**Higher Order Function** Function that takes another
...
**Lexical Scope** How R function finds *unbound* function variables: in environment
where function was created. Note: that is env, but as we know, env can change.  Lexical scope says nothing about *when* to check/evaluate.
R finds unbound `functions` in similar way. (SEE ADV-R, Chapter 6) 
*Dynamic lookup* (not scope).  Lexical is WHERE to look for unbound, not when.  Because of lazy evaluation (promises), *when* to lookup is called dynamic lookup (SEE ADV-R, Chapter 6.4.4)
**Dynamic Scope** Method to find variables in Call
Stack at runtime. Note: the R Language allows coder to
select/manipulate environment to evaluate variables. (REF: June Choe Slack/Nov 3 2021)
...
`x` is R Object
`typeof` returns character

Mode uses typeof; typeof refers to internal storage. 

SEE:  https://stat.ethz.ch/R-manual/R-devel/library/base/html/mode.html
SEE:  https://stat.ethz.ch/R-manual/R-devel/library/base/html/typeof.html
```r
q <- quote(sin(x))
mode(q)   # "call"
...
**pairlist**
formals of function are pairlist.  Can obtain/modify this pairlist.  Formal arguments become promises in the evaluation environment.   Hadley says treat like list, relic of past. Ch 18.6.1
...
**Parse** 
(TODO) Parse returns expression or creates ast structure?
Convert a string (character vector, such as "x+y") into an R Expression (ie code), which is
...
*parse_expr()* returns string, given an expression (tip:  should not do this often) Hadley 18.4
...
**promise**
Hadley: 18.4
captures expression and environment (?) Consider `promise` with 4 slots (value, expression, envir, evaluated?) Expression is raw, unevaluated code; value is result *when* evaluated.
...
**Referencial Integrety**

This is NOT referential integrity:
If x is reference to object.   And y <- x.  Two references to same object.
But if change one object, it is copied to new object with new value.  The other reference is untouched.  There are now two objects  
...
PURPOSE:  READ (https://www.milesmcbain.com/posts/the-roots-of-quotation/)
In LISP, quote is way to separate code from data.

Set variable's value to "myvalue".  The  variable 'myvar tells LISP not to evaluate myvar, not code.   quote or 'myvar returns symbolic structure of tokens/symbols or AST, unevaluated

(set 'myvar "myvalue")

(Scheme) does not look for bindings
- returns expression; stops evaluation; 
- in particular, allows programmer to then manipulate code, eval in different context
...
**quo** as a tool for evaluation to better control **evaluation
environment** whereas qenv more as a tool for reproducibility that
**captures the outcome of evaluation**.  One is "before" and the other one
is "after".

REF: Advanced-R ~ 6.5
Data structure with expression + environment.
WHY?  With dot (...) often pass several of these
rlang::eval_tidy()  # takes 1 arg (simpler)
but base:eval(, )   # requires 2 args
...
R passes by value.  (So argument is evaluated before the call?)
...
### Symbol
SEE Adv-R Chapter 18:   goal is obtain unevaluated expression to eval, manipulate, modify ...

See **R Lang Ref: 2.1.3.1**   Symbol (aka name), usually name of R object.  Use
`as.name() to coerce to symbol or quote() or atoms of parse() (Hadley: true in base R only) 
...
Easily confused with character; Hadley:  use `symbol` and not `name` which can have multiple meanins
...
# also
str(quote(y))   # symbol y

rlang::expr(y) |> is.symbol()   # T
rlang::sym("y") |> is.symbol()  # T,  sym() takes chacter, returns symbol

# b is symbol, 3 is constant, a?
ast(list(a=1, b, 3))
```
...
## Type-Safety 
R has *no* type-safety language (see:   https://stackoverflow.com/questions/14298899/type-safety-of-the-r-language ).   R is *dynamically* typed.   Exist implicit conversions between logical and numberic, for instance. 
