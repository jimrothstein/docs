...
f: RR^2 -> RR
$
== Jacobian, concise form

$
  bold(v) = (v_1, v_2, dots, v_n)
  quad
  bold(f): RR^n arrow RR^n

  J_(bold(f))(bold(v))
  =
  (frac(partial f_i, partial v_j))_(i,j=1)^n
$
...
if bond yield changes by  $delta y$, then the price change is $delta p =  "duration"/(delta y)$

Example:   If duration is 5, and the yield decreases by 100 basis points (1%), expect the price to increase by 5%

- short-term bonds respond more quickly to rate changes.
- duration of bond proportional to time to maturity (check this)
