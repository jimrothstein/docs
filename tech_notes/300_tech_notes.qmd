...
title: "300_tech_notes.qmd"
date: last-modified
description: Variety of misc tech notes
---
...
```
```
** qmd file is authority **

For pdf:
quarto render 300_tech_notes.qmd --to typst\
(in emacs M-x preview-quarto,   uses typst if it see typst chunk)

For gfm
quarto render 300_tech_notes.qmd, or  \
...
- 8/2026 install DOOM emacs (original config:  ~/.config/emacs_original)
- Need to install fd,  and NERD FONTS
...
#### GIT - Where am I
git show HEAD -- <file>  | Uses green/red to show very last changes (now included in  file)

#### History (of HEAD)  [ local only ]
git reflog
git reflog HEAD@{3}
git reflog HEAD@{yesterday}

#### Show changes made a certain <SHA> (green/red)  [ short for git log -g --short-abbrev  ]
```
git show HEAD@{3}          | commit
git show <br>@{yesterday}  | branch
````
...
git commit --amend --no-edit
  Changes last commit and changes SHA.  If already pushed, and now amending, github finds difference and won't push till you fix.
...
git switch <branch>
git switch -    | return to prior branch

git switch SHA  | no longer works
git switch --detach SHA | can inspect commit (or create new branch at this SHA)
...
#### gh commands
- gh repo delete jimrothstein/<repo> --yes     on github/Be sure have permissions( yes means no confirm )
- gh repo list
** Install R
...
put in:
- ~/.profile (for login)
- ~/.xinitrc (2/26 - setxkbmap is here)
- other locations, but get help
...
- delete all files  EXCEPT "txt" files: \
  ...
  grabs all non txt files, replaces{} with these files, all one line (+)


### which directory exists? (1st one prints)
- ls -l ~/.config/emacs/bin/doom ~/.emacs.d/bin/doom 2>/dev/null

CoMMIT first!
- find . -name '*.txt' -type f -exec rm  -i {} ;   **ask** user before removing file
- rm *.txt -i    CAREFUL  
...
- SEE https://www.howtogeek.com/192732/android-usb-connections-explained-mtp-ptp-and-usb-mass-storage/
  ...
  - adb -d push --sync * /sdcard/Music /
   -d to use emulator (when 2 devices), --sync only pushes newer files
...
### Linkedin Problem: Broken urls on a local government website, that was
created by others in the organization, source not readily available.

Solutions: Many!

Contraints:
But also constraints, non-technical staff, budget & personnel  cuts,
older hardware.

Last year:
Would have used R and packages to check website regularly for broken
urls - hardly rocket science.

This year:
Gave an LLM a brief outline of goal.  Rather than write everything out, gave the
LLM list of issues to decide or work around.  Less than 1 hour, including my
fumbling around, LLM and I came up with good approach (github actions, short
phython script, command line to run the worklow) and the LLM wrote it.

Is it perfect? Of course, not.   It can be edited by anyone comfortable with
github actions.
...
### Typst - purple ellipse

```{=typst}
#ellipse(width: 2cm, height: 1.2cm, fill: purple)
```
