...
For pdf: quarto render 300\_tech\_notes.qmd --to typst\\

For gfm quarto render 300\_tech\_notes.qmd, or\\
quarto render 300\_tech\_notes.qmd --to gfm\\

\section{TOD0}\label{tod0}
...
LLM NOTES? USE: 320\_LLM\_agents\_NOTES.md
...
\subsection{url links}\label{url-links}
...
\subsection{Use Calibre for all files - one central place - including
those currently on
...
\textbf{Feb 2026} Haskell working hard on data science;
tutorials\ldots{} cloud based GUI

\textbf{Dec 2025} - HP boot reports amdgpu psp gfx failed, but continues
...
\subsection{Chron}\label{chron}

\textless2024-10-10 Thu\textgreater{} now Debian 12 (Cinnamon or i3); no
more linux mint; LMDE \ldots{} \textless2024-08-26 Mon\textgreater{} -
do not use emacs as ``DESKTOP'' - everything, windows on the screen at
once. - humans can not multitask; keep window only used for current
work. - computers great for keeping lists of buffers, search when you
move to next task. - tools like ivy can recall things like split window,
code 1 side; dir on other - exist tools to maintain buffers in
`projects' - EXWM still not sure.
...
\textbf{IMPERATIVE v DECLARATIVE} - math notation is DECLARATIVE;
indicates meaning, not exact details (example1: summation sign). Tell
computer what you WANT, leave details to compiler or program. -
...
\subsection{section\{LATEX NOTES\}}\label{sectionlatex-notes}
...
\subsection{revealjs slides with
...
\subsubsection{gh commands}\label{gh-commands}

\begin{itemize}
\tightlist
\item
  gh repo delete jimrothstein/ on github/Be sure have permissions
\item
...
\section{\textasciitilde/code/docs/tech\_notes/001\_grep\_regex\_P\_examples.qmd}\label{codedocstech_notes001_grep_regex_p_examples.qmd}

\section{\textasciitilde/code/docs/tech\_notes/002\_grep\_examples.md}\label{codedocstech_notes002_grep_examples.md}

\section{SOME regex: in
...
\subsection{DEFINITIONS - as always,
...
\subsection{Render REGEX Verbatim - 4 ways
...
\subsection{Specific to R}\label{specific-to-r}
...
\subsection{LINUX / DEBIAN}\label{linux-debian}

\subsubsection{lightdm/login}\label{lightdmlogin}
...
\item
  /etc/profile
\item
  \textasciitilde/.profile
\end{itemize}

\subsubsection{Xsession}\label{xsession}

\begin{itemize}
\tightlist
\item
  look /etc/X11/Xsession and \ldots/Xsession.d
\item
...
\section{CAPSLOCK}\label{capslock}

\subsection{setxkbmap}\label{setxkbmap}

put in: - \textasciitilde/.profile (for login) -
\textasciitilde/.xinitrc (2/26 - setxkbmap is here) - other locations,
but get help

\subsection{Xmodmap (older, easier ?)}\label{xmodmap-older-easier}
...
\subsection{SETUP/CONFIG new HP Elitebook
machine}\label{setupconfig-new-hp-elitebook-machine}

\textless2024-10-10 Thu\textgreater{} HP Elitebook but with pure
Debian/Cinnamon \& i3
...
\subsection{Chromebox}\label{chromebox}

PURPOSE: \textbf{maps ChromeBox ``capslock'' key to Escape.} - use
\textgreater{} xev to find that capslock is key 133. - xmodmap is older,
but simpler to change key action to change key action. - newer is
\textbf{setxkbmap} but I find more effort to figure out simple things. -
SEE tech\_notes - lots of ways to do this remap. This works, stay with
it:

\subsection{Lenovo}\label{lenovo}
...
\section{ZSH notes}\label{zsh-notes}

ZSH REF CARD: (start) https://www.bash2zsh.com/zsh\_refcard/refcard.pdf
...
\subsection{script header}\label{script-header}

\#!/bin/zsh -xv \# verbose

\subsection{diff -y file1 file2 (2
...
\subsection{bind to widget}\label{bind-to-widget}
...
\subsection{glob vs extended-glob (SEE Guide
...
\subsection{parameter expansion}\label{parameter-expansion}
...
\subsection{zsh/unix permissions}\label{zshunix-permissions}
...
\subsection{basic linux commands}\label{basic-linux-commands}
...
CoMMIT first! - find . -name '\emph{.txt' -type f -exec rm -i \{\} ;
\textbf{ask} user before removing file - rm }.txt -i CAREFUL

\subsection{zsh commands in emacs (SEE:
zsh\_project)}\label{zsh-commands-in-emacs-see-zsh_project}

\subsection{completion, Use zstyle}\label{completion-use-zstyle}
...
\subsection{sudo vs su \ldots.}\label{sudo-vs-su-.}
...
\subsection{drive info}\label{drive-info}
...
\subsection{BINDKEY}\label{bindkey}
...
\subsection{Google, API, curl}\label{google-api-curl}
...
\subsection{From Explorer}\label{from-explorer}
...
\subsection{From Google Playground}\label{from-google-playground}
...
\subsection{HTTR2 - NOTES (needs clean
...
\subsection{MORE CURL}\label{more-curl}
...
\subsection{Android, Mobile, Cell Phone -
...
\subsection{section\{Laptop Buying
...
\subsection{Battery Notes (3/1/2026)}\label{battery-notes-312026}
...
\subsection{Reddit, API}\label{reddit-api}
...
\subsection{Method 2: Can you tell an AI Agent to do
...
\subsection{Method 3: Dedicated Third-Party Tools \&
