...
\section{Jul 2026}\label{jul-2026}
...
\subsection{sudo apt}\label{sudo-apt}

sudo apt autoremove \# a dependency no longer needed sudo apt-mark \#
pkgs settings, such as being held sudo apt install --fix-broken sudo apt
install --fix-missing

\section{linux cli}\label{linux-cli}
...
\subsection{PDF Notes {[}ignores html, css; also ignores YAML header
(pandoc \&
::render(){]}}\label{pdf-notes-ignores-html-css-also-ignores-yaml-header-pandoc-render}

\subsection{Print raw text (example, from github,
...
\section{DOCUMENT REGEX HERE}\label{document-regex-here}

\section{(text, no examples in this
...
\subsection{Specific to vim/neovim}\label{specific-to-vimneovim}
