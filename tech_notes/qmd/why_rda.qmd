...
## Working with .rda (.Rdata) files - NOTES

1. **Why use \*.rda?**
   Data included in package must be *.rda format; CRAN checks (?) 
...
2. The **load() function** retrieves an \*.rda dataset as a side-effect.  It returns it to the current environment (typically globalenv()) and clobber existing file with same name.   The load() function can cause confusion and one reason why people often prefer *.rds. [`loadRDS()`  behaves the way we expect for *.rds files].
...
my favorite uses `call`
create & save call object; later use `eval`
```
...
5. To **minimize confusion**, therefore, I want to suggest picking a common way to saving and retrieving intermediate *.rda files.
