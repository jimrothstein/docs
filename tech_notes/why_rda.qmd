...
### Two key points
- ~~Admiral~~ **Pharmaverse** uses `*.rda` (and not `.rds`) because data in packages must be in this form.  There is more flexibility with intermediate or prepared datasets, but admiral continues with `.rda` 
- As to loading `*.rda` datasets, ~~admiral~~ **Pharmaverse** uses a few different ways and there does not appear any standardization.
...
### A few  examples of ~~admiral~~ **Pharmaverse** code to load `*.rda` files
...
from pharamaverseadam/create_adams_data.R
```
...
4. NOTE:  For files included in package, the admiral is using an easy way: 
```
...
5. *To minimize confusion,  I suggest picking a common way to save and retrieve intermediate *.rda files.*
