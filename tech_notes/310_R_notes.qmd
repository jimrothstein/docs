...
**Expression** Interactively, a valid R statement that returns an value is  called an expression,
An expression() takes an R object and returns expression, which a
R list.  Note:  a `statement` is code that may do things (side effects) but
does not return anything.
Hadley defines expression as an "object that stores quoted code without evaluating it." (tidyr cheat sheet)
SEE evaluation.

**Expression v Language** R expressions are based on list and can be broken down
