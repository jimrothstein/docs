...
= Misc
In emacs, open "frame" then 1 or more "windows" inside the frame
= i3 + emacs
This works (Jan 2026)
i3 conf has "emacs --daemon"  (runs in  background);
Then, when I begin work,  emacsclient -a "" -t  (terminal)
emacsclient -a "" -c   (gui, such as menus)
...
== Kill emacs server
emacsclient -e '(kill-emacs)',   OR
^x-^c '(client-save-kill-emacs'
